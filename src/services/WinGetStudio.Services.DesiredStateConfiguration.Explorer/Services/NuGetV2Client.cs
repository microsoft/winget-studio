...
internal sealed partial class NuGetV2Client : INuGetV2Client
{
    ...
    private readonly ILogger<NuGetV2Client> _logger;

    public NuGetV2Client(HttpClient http, INuGetV2Parser parser, ILogger<NuGetV2Client> logger)
    {
        _http = http;
        _parser = parser;
        _logger = logger;
    ...
    public async Task<IReadOnlyList<ModuleMetadata>> SearchAsync(string baseUrl, NameValueCollection query)
    {
        ...
        {
            _logger.LogError(e, "Failed to search DSC modules from PowerShell Gallery.");
            throw;
        }
    }
}
