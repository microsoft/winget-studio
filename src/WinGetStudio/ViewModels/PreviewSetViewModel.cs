...
public delegate PreviewSetViewModel PreviewSetViewModelFactory();

public sealed partial class PreviewSetViewModel : ObservableObject
{
    private readonly ILogger<PreviewSetViewModel> _logger;
    private readonly IStringLocalizer<PreviewSetViewModel> _localizer;
    ...
    public PreviewSetViewModel(
        ILogger<PreviewSetViewModel> logger,
        IStringLocalizer<PreviewSetViewModel> localizer,
    ...
    private async Task OnUpdateSelectedUnitAsync()
    {
        ...
        {
            _logger.LogInformation($"Updating selected unit");
            await UpdateUnitInternalAsync(SelectedUnit.Item1, SelectedUnit.Item2);
        }
    }
    ...
    {
        IsEditMode = true;
        await SelectUnitAsync(unit);
    }

    private async Task SelectUnitAsync(UnitViewModel unit)
    {
    ...
    private async Task<bool> UpdateUnitInternalAsync(UnitViewModel original, UnitViewModel updated)
    {
        if (IsConfigurationLoaded)
        {
            try
